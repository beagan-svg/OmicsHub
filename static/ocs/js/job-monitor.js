/**
 * job-monitor.js
 * Handles functionality for job monitoring
 */
...
class JobMonitor {
    ...
    /**
     * Click-to-copy for ".copy-cell" is handled app-wide by copyCell.js
     * (ripple + copied pulse + toast) via event delegation, so dynamically
     * rendered cells work without re-binding. Kept as a no-op so existing
     * call sites stay valid.
     */
    initializeCopyToClipboard() { /* handled globally by copyCell.js */ }

    /**
     * Status badge markup matching the shared status_badge.html component
     * (icon + label, shared .status-badge classes). Unknown/empty -> NOT
     * COMPLETED. One mapping for the completed-jobs table.
     */
    statusBadgeHtml(status, title) {
        const s = (status || '').toLowerCase();
        const t = title ? ` title="${title}"` : '';
        let cls, icon, label;
        if (s === 'completed' || s === 'complete') { cls = 'status-completed'; icon = 'bi-check-circle-fill'; label = 'Completed'; }
        else if (s === 'in progress' || s === 'in-progress' || s === 'running') { cls = 'status-in-progress'; icon = 'bi-arrow-clockwise'; label = 'In Progress'; }
        else if (s === 'failed' || s === 'error') { cls = 'status-failed'; icon = 'bi-x-circle-fill'; label = 'Failed'; }
        else if (s === 'aborted' || s === 'stopped' || s === 'killed') { cls = 'status-stopped'; icon = 'bi-slash-circle-fill'; label = status; }
        else if (s === 'pending' || s === 'submitted' || s === 'queued' || s === 'ready') { cls = 'status-pending'; icon = 'bi-clock-fill'; label = status; }
        else if (s && s !== 'not started' && s !== 'not completed') { cls = 'status-pending'; icon = 'bi-clock-fill'; label = status; }
        else { cls = 'status-not-completed'; icon = 'bi-circle'; label = 'Not Completed'; }
        return `<span class="status-badge ${cls}"${t}><i class="bi ${icon}"></i><span class="status-text">${label}</span></span>`;
    }
    ...
    renderCompletedJobsTable(jobs) {
        ...
        processedJobs.forEach(job => {
            ...
            // Alignment / Post-QC status badges via the shared mapping.
            const alignmentStatus = job.alignment_status || 'Not Completed';
            let alignmentTitle = 'Alignment status: ' + alignmentStatus;
            if ((alignmentStatus === 'COMPLETED' || alignmentStatus === 'Completed') && job.alignment_end_time) {
                alignmentTitle = 'Alignment completed at ' + this.formatDate(job.alignment_end_time);
            }
            const alignmentStatusHtml = this.statusBadgeHtml(alignmentStatus, alignmentTitle);

            const postqcStatus = job.postqc_status || 'Not Completed';
            let postqcTitle = 'Post-QC status: ' + postqcStatus;
            if ((postqcStatus === 'COMPLETED' || postqcStatus === 'Completed') && job.postqc_end_time) {
                postqcTitle = 'Post-QC completed at ' + this.formatDate(job.postqc_end_time);
            }
            const postqcStatusHtml = this.statusBadgeHtml(postqcStatus, postqcTitle);
                    ...
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-2">
                                ...
                                ${this.statusBadgeHtml(job.alignment_status)}
                            </div>
                            ${job.alignment_demand_id ? `
                                <div class="mb-2">
                ...
                <div class="col-md-6 mb-3">
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-2">
                                <strong>Status:</strong>
                                ${this.statusBadgeHtml(job.postqc_status)}
                            </div>
                            ${job.postqc_demand_id ? `
                                <div class="mb-2">
                                    <strong>Demand ID:</strong> 
