...
CSV_DIR="$PROJECT_ROOT/import_ocs/data/csv"
...
# Output metadata file - now in the proper directory
metadata_file="$CSV_DIR/isilon.csv"

# Column headers for the metadata file
header="Fastq Name,Library Prep Method,Study Set,Alignment Method,Amplification ID,Amplification,Batch Name,Batch Name From Vendor,Cell Capture,Cell Prep Type,Library Prep Method ID,Library Prep Name,Load Name,Organism Common Name,Organism Name,Sample ID,Sample Name,Sample Type,Sequencing Vendor"
echo "$header" > "$metadata_file"

# Fetch all batch metadata
json_output=$(ocs batches list isilon-s3-backup --format json)
...
echo "CSV data saved to $metadata_file"
echo "Done!"
