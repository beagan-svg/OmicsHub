...
#
# Usage:
#   ./import_ocs/shell/run_vendor_data_collection.sh [OPTIONS]
#
# Options:
...
#
# Examples:
#   ./import_ocs/shell/run_vendor_data_collection.sh --collect isilon
#   ./import_ocs/shell/run_vendor_data_collection.sh --all
#
...
DATA_DIR="$IMPORT_OCS_DIR/data/csv"
LOG_DIR="$IMPORT_OCS_DIR/logs"
...
run_vendor_script() {
    ...
    if [ -f "$output_csv" ]; then
        ...
        log "WARNING" "$vendor.csv was not created in $DATA_DIR!"
        return 1
    fi
...
import_data() {
    local source=$1
    local import_script="$IMPORT_OCS_DIR/data_import/import_vendor_data.py"
