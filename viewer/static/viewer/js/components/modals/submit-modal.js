...
class PipelineSubmitModal {
    ...
    setupStyles() {
        ...
        if (!document.getElementById('submit-modal-styles')) {
            const style = document.createElement('style');
            style.id = 'submit-modal-styles';
            style.textContent = `
                .code-block {
                    max-height: 80px;
                    overflow-y: auto;
    ...
    populateModal() {
        ...
        selectedSamples.forEach(sample => {
            ...
            if (ingestStatus.toLowerCase() === 'not started') {
                ...
                this.postQCSamples.push({ ...sample, fastqName, workflow });
            } else if (ingestStatus.toLowerCase() === 'completed' && alignmentStatus.toLowerCase() !== 'completed') {
                this.alignmentSamples.push({ ...sample, fastqName, workflow });
            }
    ...
    async handleSubmission() {
        ...
        try {
            ...
            if (samplesToSubmit.length === 0) {
                throw new Error('No samples selected for submission');
            }

            // Get auto-proceed value
            const autoProceed = this.autoProceedToggle.checked;

            // Use the API client to submit samples
            const response = await window.API.submitSamples(samplesToSubmit, autoProceed);

            // Hide modal
            modalInstance.hide();

            // Show success notification
            window.AppUtils.showToastNotification('Samples submitted successfully', 'success');

            // Clear selected samples from local storage
            if (window.pipelineLocalData) {
                window.pipelineLocalData.clearSelectedSamples();
            }
        } catch (error) {
            // Show error notification
            window.AppUtils.showToastNotification(
...
// Initialize the modal handler when the document is ready
