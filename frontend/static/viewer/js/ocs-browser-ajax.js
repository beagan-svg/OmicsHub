...
const Utils = {
    ...
    logDebug(message, data) {
        if (PRODUCTION_CONFIG.debug) {
            if (data !== undefined) {
                console.log(`[Browser AJAX] ${message}`, data);
            } else {
                console.log(`[Browser AJAX] ${message}`);
            }
        }
    },
...
const APIManager = {
    ...
    async fetchSamples(params = {}) {
        console.log('[Browser AJAX] APIManager.fetchSamples() called with params:', params);
        ...
        console.log('[Browser AJAX] Making request to URL:', url);
        ...
        console.log('[Browser AJAX] API response received:', result);
        return result;
    },
    ...
    async fetchFilterOptions() {
        try {
            const url = `${PRODUCTION_CONFIG.apiBaseUrl}?action=filter-options`;
            return await this.makeRequest(url);
        } catch (error) {
            Utils.logDebug('Failed to fetch filter options from API:', error);
            return {
                batch_rtx: [],
                batch_mtx: [],
                batch_atx: [],
                ...
                library_prep_methods: [],
                ingest_status_options: [],
                alignment_status_options: [],
                postqc_status_options: []
            };
        }
    }
};
...
const TableManager = {
    ...
    createSampleRow(sample) {
        ...
        const cells = [
            ...
            this.createCell('text', sample.batch_name, 'field-batch_name'),
    ...
    createStatusBadge(status) {
        ...
        if (!status) {
            badge.className += ' status-not-completed';
            badge.innerHTML = '<i class="bi bi-circle"></i><span class="status-text">NOT STARTED</span>';
            return badge;
        }
        ...
        badge.title = status;
        badge.innerHTML = `<i class="bi ${icon}"></i><span class="status-text">${Utils.escapeHtml(displayText)}</span>`;
...
const FilterDataManager = {
    ...
    async loadCompleteDataset() {
        console.log('[FilterDataManager] Loading complete dataset...');
        ...
        try {
            ...
            console.log(`[FilterDataManager] Database contains ${totalRecords} total records`);
            ...
            if (totalRecords > 0 && totalRecords <= UI_CONSTANTS.CLIENT_SIDE_MAX_RECORDS) {
                console.log('[FilterDataManager] Loading all records for client-side filtering...');
                ...
                allSamples = response.samples || response.results || [];

                if (allSamples.length < totalRecords) {
                    console.warn(`[FilterDataManager] Only loaded ${allSamples.length} of ${totalRecords} records. Some may be missing.`);

                    // Try alternative approach: load in chunks
                    console.log('[FilterDataManager] Attempting to load remaining records in chunks...');
                    const chunkSize = UI_CONSTANTS.CHUNK_SIZE;
                    const chunks = Math.ceil(totalRecords / chunkSize);

                    for (let i = 1; i <= chunks; i++) {
                        try {
                            const chunkResponse = await APIManager.fetchSamples({
                                page: i,
                                per_page: chunkSize
                            });
                            const chunkSamples = chunkResponse.samples || chunkResponse.results || [];

                            if (i === 1) {
                                allSamples = chunkSamples; // Replace initial data
                            } else {
                                allSamples = allSamples.concat(chunkSamples);
                            }

                            console.log(`[FilterDataManager] Loaded chunk ${i}/${chunks}: ${chunkSamples.length} records`);
                        } catch (chunkError) {
                            console.error(`[FilterDataManager] Error loading chunk ${i}:`, chunkError);
                            break;
                        }
                    }
                }
                ...
                console.log(`[FilterDataManager] Dataset too large (${totalRecords} records). Falling back to server-side filtering.`);
                ...
                console.log('[FilterDataManager] No records found or unable to determine total count');
                allSamples = [];
            }
            ...
            console.log(`[FilterDataManager] Successfully loaded ${this.state.allData.length} records for client-side filtering`);
    ...
    extractFilterOptions() {
        console.log('[FilterDataManager] Extracting filter options from data...');
        ...
        console.log('[FilterDataManager] Filter options extracted:', this.state.filterOptions);
    },
...
const FilterLogicManager = {
     ...
     */
    applyAllFilters() {
        console.log('[FilterLogicManager] Applying all filters client-side...');
        const startTime = PRODUCTION_CONFIG.debug ? performance.now() : null;
            ...
            if (selectedValues && selectedValues.length > 0) {
                filtered = filtered.filter(sample => {
                    ...
                    if (filterName === 'batch_rtx' || filterName === 'batch_mtx' || filterName === 'batch_atx') {
                        ...
                        const sampleValue = sample[filterName] || sample[filterName.replace(/s$/, '')];
                        return selectedValues.includes(sampleValue);
                    }
                });
            }
        });
        ...
        if (PRODUCTION_CONFIG.debug && startTime) {
            const duration = performance.now() - startTime;
            console.log(`[FilterLogicManager] Client-side filtering completed in ${duration.toFixed(2)}ms`);
        }
        console.log(`[FilterLogicManager] Filtered ${FilterDataManager.state.allData.length} → ${FilterDataManager.state.filteredData.length} records`);
    ...
    handleFilterChange(selectElement) {
        ...
        console.log(`[FilterLogicManager] Filter changed: ${filterName}`, selectedValues);
        console.log(`[FilterLogicManager] Current filter mode: ${FilterDataManager.state.filterMode}`);
        ...
        if (FilterDataManager.state.filterMode === 'realtime') {
            console.log(`[FilterLogicManager] Real-time mode active - applying filters automatically`);
            this.applyAllFilters();
        } else {
            console.log(`[FilterLogicManager] Manual mode active - filters will not be applied automatically`);
        }
...
const FilterUIManager = {
    ...
    async initializeSearchInput() {
        ...
        const debouncedSearch = Utils.debounce((searchTerm) => {
            console.log(`[FilterUIManager] Search triggered: "${searchTerm}"`);
    ...
    async initializeFilterDropdowns() {
        console.log('[FilterUIManager] Initializing filter dropdowns...');
    ...
    populateFilterDropdown(selectId, options) {
        ...
        console.log(`[FilterUIManager] Populating ${selectId} with ${options.length} options:`, options);
        ...
        console.log(`[FilterUIManager] Counting from ${allData.length} total records for field: ${selectId}`);
        ...
        console.log(`[FilterUIManager] Count map for ${selectId}:`, countMap);
        ...
        console.log(`[FilterUIManager] Successfully populated ${selectId} with ${options.length} options`);
    },
    ...
    updateActiveFiltersDisplay() {
        ...
        console.log('[FilterUIManager] Updating active filters display');
        console.log('- Search term:', BrowserAJAX.state.searchTerm);
        console.log('- Active filters:', BrowserAJAX.state.activeFilters);
        console.log('- Filtered data length:', FilterDataManager.state.filteredData.length);
        console.log('- All data length:', FilterDataManager.state.allData.length);
        ...
        if (hasActiveFilters || FilterDataManager.state.filteredData.length !== FilterDataManager.state.allData.length) {
            ...
            console.log('[FilterUIManager] Filter container shown with', container.children.length, 'elements');
        } else {
            // Hide container when no filters
            container.style.display = 'none';
            container.style.visibility = 'hidden';
            container.style.opacity = '0';
            console.log('[FilterUIManager] Filter container hidden - no active filters');
        }
    },
    ...
    createFilterTag(container, filterName, value, displayText) {
        ...
        removeButton.addEventListener('click', (e) => {
            ...
            console.log(`[FilterUIManager] Removing filter tag: ${filterName} = ${value}`);
    ...
    removeFilterTag(filterName, value) {
        console.log(`[FilterUIManager] Removing filter: ${filterName} = ${value}`);
...
const FilterStateManager = {
    ...
    updateFilterState() {
        ...
        if (state.filterMode && state.filterMode !== 'realtime') {
            url.searchParams.set('filterMode', state.filterMode);
        }
    ...
    async restoreFilterState() {
        console.log('[FilterStateManager] Restoring filter state...');
        ...
        if (hasUrlParams) {
            ...
            if (saved) {
                try {
                    ...
                    FilterDataManager.state.filterMode = state.filterMode || 'realtime';
        ...
        console.log('[FilterStateManager] Filter state restored. Filter mode:', FilterDataManager.state.filterMode);
    }
};
...
const FilterExtrasManager = {
    ...
    updateFilterStatistics() {
        ...
        console.log('[FilterExtrasManager] Filter statistics:', stats);
    },
...
const FilterManager = {
     ...
     */
    async init() {
        console.log('[FilterManager] Initializing enhanced filtering system...');
        ...
        try {
            ...
            setTimeout(() => {
                ...
                console.log('[FilterManager] Active filters display updated after initialization');
            }, 300);

            console.log('[FilterManager] Filter system initialized successfully');
            ...
            if (this.data.state.filterMode === 'server-side') {
                console.log('[FilterManager] Continuing with server-side filtering...');
    ...
    // FilterManager-specific methods (not delegated)
    updateFilterModeToggleState() {
        ...
        if (toggle) {
            ...
            console.log('[FilterManager] Updated toggle state - checked:', toggle.checked, 'disabled:', toggle.disabled);
        }
        ...
        if (label) {
            ...
            console.log('[FilterManager] Updated label text:', modeInfo);
        }
    },
    ...
    attachFilterModeToggleListener() {
        ...
        if (toggle.disabled) {
            console.log('[FilterManager] Filter mode toggle is disabled (server-side mode)');
            return;
        }

        console.log('[FilterManager] Attaching event listener to filter mode toggle');
        console.log('[FilterManager] Current toggle state - checked:', toggle.checked, 'filter mode:', this.state.filterMode);
        ...
        updatedToggle.addEventListener('change', (e) => {
            console.log('[FilterManager] Filter mode toggle changed:', e.target.checked);

            const oldMode = this.state.filterMode;
            this.state.filterMode = e.target.checked ? 'realtime' : 'manual';

            console.log('[FilterManager] Filter mode changing from', oldMode, 'to', this.state.filterMode);
            ...
            if (label) {
                ...
                const modeInfo = this.state.filterMode === 'realtime'
                    ...
                    : 'Manual';
                label.textContent = modeInfo;
                console.log('[FilterManager] Updated label to:', modeInfo);
            } else {
                console.warn('[FilterManager] Could not find .filter-mode-status label to update');
            }

            // Apply filters immediately if switching to real-time mode
            if (this.state.filterMode === 'realtime' && oldMode !== 'realtime') {
                console.log('[FilterManager] Switching to real-time mode, applying filters...');
                this.applyAllFilters();
            } else if (this.state.filterMode === 'manual' && oldMode === 'realtime') {
                console.log('[FilterManager] Switched to manual mode - filters will not apply automatically');
            }
            ...
            console.log('[FilterManager] Filter mode successfully changed from', oldMode, 'to', this.state.filterMode);
        });

        console.log('[FilterManager] Filter mode toggle listener attached successfully');
    },
    ...
    async initializeAdvancedFilters() {
        ...
        console.log('[FilterManager] Advanced filters panel initialization delegated to App.initializeButtonEventIsolation()');
    },
    ...
    initializeFilterModeToggle() {
        console.log('[FilterManager] Initializing filter mode toggle...');
        ...
        console.log('[FilterManager] Container found:', container.className);
...
const SelectionManager = {
    ...
    extractSampleData(row) {
        return {
            ...
            ingest_status: row.dataset.ingestStatus || 'Not Started',
            alignment_status: row.dataset.alignmentStatus || 'Not Started',
            postqc_status: row.dataset.postqcStatus || 'Not Started'
        };
    },
    ...
    async sendToPipeline() {
        ...
        try {
            ...
            console.log('[Browser] === SEND TO PIPELINE START ===');
            console.log('[Browser] New samples to send:', BrowserAJAX.state.selectedSamples.length);
            console.log('[Browser] Sample names:', BrowserAJAX.state.selectedSamples.map(s => s.fastq_name || s.fastq || s.fastq_id));
            ...
            if (existingData) {
                try {
                    ...
                    if (Array.isArray(existingSamples)) {
                        ...
                        console.log('[Browser] Found existing samples:', existingSamples.length);
                        console.log('[Browser] Existing sample names:', existingSamples.slice(0, 5).map(s => s.fastq_name || s.fastq || s.fastq_id));
                    }
                } catch (parseError) {
                    console.error('[Browser] Error parsing existing data:', parseError);
                }
            } else {
                console.log('[Browser] No existing samples found in localStorage');
            }
            ...
            console.log('[Browser] Existing fastq names:', existingFastqNames.size, 'unique samples');
            ...
            BrowserAJAX.state.selectedSamples.forEach(newSample => {
                ...
                if (fastqName && !existingFastqNames.has(fastqName)) {
                    ...
                    skippedCount++;
                    if (fastqName) {
                        console.log('[Browser] Skipping duplicate sample:', fastqName);
                    } else {
            ...
            console.log('[Browser] Merge results:');
            console.log('  - Samples added:', addedCount);
            console.log('  - Samples skipped (duplicates):', skippedCount);
            console.log('  - Total samples after merge:', mergedSamples.length);

            // Store the merged samples in localStorage
            localStorage.setItem(PRODUCTION_CONFIG.storageKey, JSON.stringify(mergedSamples));
            console.log('[Browser] Merged samples saved to localStorage');
            ...
            Utils.showMessage(`${addedCount} new samples added to Pipeline Dashboard (${skippedCount} duplicates skipped)`, 'success', 3000);

            // Wait a bit more for user to see the success state
            await new Promise(resolve => setTimeout(resolve, 1500));

            console.log('[Browser] === SEND TO PIPELINE END ===');
 ...
 */
const ColumnManager = {
    // Store previous column states for show all toggle
    previousColumnStates: null,
    // Flag to prevent circular updates during manual toggle
    ...
    loadColumnSettings() {
        const defaultSettings = {
            // Sample Information (minimal set visible)
            fastq_name: true,
            study_set: true,
            load_name: true,
            batch_name: false,
            ...
            // Sample Characteristics (hidden by default for cleaner initial view)
            organism: false,
            ...
            // Amplification (default hidden)
            amplification_name: false,
            amplification_id: false,

            // Library (only the main method visible)
            library_prep_method: true,
            library_prep_method_id: false,
            library_prep_name: false,
            alignment_method: false,

            // Processing Status (visible by default - core workflow info)
            ingest_status: true,
            alignment_status: true,
            postqc_status: true,

            // Status FID (hidden by default)
            ingest_fid: false,
            alignment_fid: false,
            postqc_fid: false,

            // Status Times (hidden by default)
        ...
        BrowserAJAX.state.columnSettings = StateManager.load('columnSettings', defaultSettings);
        this.applyColumnSettings();
    },

    saveColumnSettings() {
        StateManager.save('columnSettings', BrowserAJAX.state.columnSettings);
    },
    ...
    handleShowAllToggle(showAll) {
        console.log('[Browser AJAX] Show all toggle clicked:', showAll);
        console.log('[Browser AJAX] Current columnSettings:', BrowserAJAX.state.columnSettings);
        console.log('[Browser AJAX] Previous states before toggle:', this.previousColumnStates);

        // Set flag to prevent circular updates
        this.isManualToggle = true;

        if (showAll) {
            // Only save previous states if we're not already showing all
            const currentlyAllVisible = Object.values(BrowserAJAX.state.columnSettings).every(visible => visible);

            if (!currentlyAllVisible) {
                // Save current column states before showing all
                this.previousColumnStates = JSON.parse(JSON.stringify(BrowserAJAX.state.columnSettings));
                console.log('[Browser AJAX] Saved previous column states:', this.previousColumnStates);
            } else {
                console.log('[Browser AJAX] All columns already visible, keeping existing previous states');
            }
            ...
            // Restore previous column states
            console.log('[Browser AJAX] Attempting to restore previous states:', this.previousColumnStates);

            if (this.previousColumnStates && Object.keys(this.previousColumnStates).length > 0) {
                console.log('[Browser AJAX] Restoring previous column states:', this.previousColumnStates);

                // Restore the settings
                BrowserAJAX.state.columnSettings = JSON.parse(JSON.stringify(this.previousColumnStates));

                // Update individual checkboxes to match restored states
                document.querySelectorAll('.column-toggle-checkbox').forEach(checkbox => {
                    const columnName = checkbox.dataset.column;
                    if (columnName && this.previousColumnStates.hasOwnProperty(columnName)) {
                        checkbox.checked = this.previousColumnStates[columnName];
                        console.log('[Browser AJAX] Setting checkbox for', columnName, 'to', this.previousColumnStates[columnName]);
                    }
                });
                ...
                // If no previous states exist, restore to default configuration instead of hiding all
                console.log('[Browser AJAX] No previous states found, restoring to defaults');

                BrowserAJAX.state.columnSettings = {
                    // Sample Information (default visible)
                    fastq_name: true,
                    study_set: true,
                    load_name: true,
                    batch_name: false,
                    ...
                    // Library (some visible by default)
                    library_prep_method: true,
                    library_prep_method_id: false,
                    library_prep_name: false,
                    alignment_method: false,

                    // Processing Status (visible by default)
                    ingest_status: true,
                    alignment_status: true,
                    postqc_status: true,
                ...
                document.querySelectorAll('.column-toggle-checkbox').forEach(checkbox => {
                    const columnName = checkbox.dataset.column;
                    if (columnName && BrowserAJAX.state.columnSettings.hasOwnProperty(columnName)) {
                        checkbox.checked = BrowserAJAX.state.columnSettings[columnName];
                    }
                });

                Utils.showMessage('Restored to default column visibility', 'info');
            }

            // Clear previous states after restoration
            this.previousColumnStates = null;
        }

        console.log('[Browser AJAX] Final columnSettings after toggle:', BrowserAJAX.state.columnSettings);

        // Apply the column settings and save
        this.applyColumnSettings();
        this.saveColumnSettings();

        // Clear the manual toggle flag
    ...
    updateShowAllCheckboxState() {
        // Don't update during manual toggle to avoid overriding user action
        if (this.isManualToggle) {
            console.log('[Browser AJAX] Skipping automatic checkbox update during manual toggle');
            return;
        }
        ...
        console.log('[Browser AJAX] Updating show all checkbox state - visible:', visibleColumns, 'total:', totalColumns);
     ...
     */
    resetToDefaults() {
        // Clear any saved previous states
        this.previousColumnStates = null;

        // Reset to default settings - show only essential columns
        BrowserAJAX.state.columnSettings = {
            // Sample Information (minimal set visible)
            fastq_name: true,
            study_set: true,
...
const SortingManager = {
    init() {
        console.log('[SortingManager] Initializing sorting functionality');
        this.setupEventHandlers();
    ...
    applySortToFilteredData() {
        if (!FilterManager.state.filteredData || FilterManager.state.filteredData.length === 0) {
            console.log('[SortingManager] No data to sort');
            return;
        }

        console.log(`[SortingManager] Sorting filtered data by ${BrowserAJAX.state.sortField} ${BrowserAJAX.state.sortDirection}`);
...
const App = {
    ...
    initializeButtonEventIsolation() {
        console.log('[Browser AJAX] Initializing button event isolation...');
        ...
        if (advancedFiltersBtn) {
            ...
            newBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                console.log('[Browser AJAX] Advanced Filters button clicked');
                ...
                if (filtersPanel) {
                    ...
                    if (isHidden) {
                        ...
                        console.log('[Browser AJAX] Advanced filters panel opened');
                    } else {
                        // Hide the panel
                        ...
                        console.log('[Browser AJAX] Advanced filters panel closed');
                    }
                }
            });
            ...
            if (wasOpen && filtersPanel) {
                ...
                newBtn.classList.remove('btn-outline-primary');
                newBtn.classList.add('btn-primary');
                console.log('[Browser AJAX] Advanced filters restored to open state');
                ...
                newBtn.classList.remove('btn-primary');
                newBtn.classList.add('btn-outline-primary');
                console.log('[Browser AJAX] Advanced filters set to closed state');
            }
        }
        ...
        if (columnSettingsBtn) {
            ...
            columnSettingsBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                console.log('[Browser AJAX] Column Settings button clicked');
        ...
        console.log('[Browser AJAX] Button event isolation initialized');
    },
