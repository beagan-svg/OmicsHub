...
class RequestIDMiddleware:
    ...
    def __call__(self, request):
        ...
        # Deliberately not reset on the way out. Django's "Internal Server Error: /path"
        # log line for every 5xx is written from `BaseHandler.get_response` after the
        # middleware chain has returned, so the request ID must still be set at that
        # point. Resetting here stamps that line with "-" and breaks the trail at
