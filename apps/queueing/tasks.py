...
from apps.catalog.models import StageStatus
from apps.catalog.services import sync
from apps.ocs import cli, dynamodb
from apps.queueing.models import QueueEntry
from apps.queueing.services.claim import (
    claim_next_entry,
    record_failure,
    record_stranded,
    record_submission,
...
CAPACITY_HOLD_KEY = "queueing:capacity-hold"
...
SPACING_HOLD_KEY = "queueing:spacing-hold"
...
# Refreshed on every run of the task below, and read by the health endpoint. Beat calls the
# task every 60 seconds, so a key this old means at least four ticks produced nothing —
# beat is down, the broker is not delivering, or nothing is consuming `submissions`. Any of
# those is the same outage: entries pile up as PENDING and no submission reaches OCS.
WORKER_HEARTBEAT_KEY = "queueing:submissions-worker-seen"
...
def _hold_for_spacing(seconds: int) -> None:
    ...
    if seconds:
        cache.set(SPACING_HOLD_KEY, True, timeout=seconds)
...
def process_next_queue_entry():
    ...
    # reads — beat published, the broker delivered, and this worker consumed it.
    ...
    if in_progress >= job_settings["limit"]:
        ...
        cache.set(CAPACITY_HOLD_KEY, True, timeout=hold_for)
        logger.info(
...
def reconcile_stranded_submissions():
    """Move queue entries abandoned during submission out of SUBMITTING.

A SUBMITTING entry means the worker claimed it and then stopped, because it was killed,
OOMed, or redeployed, somewhere around the `ocs` call. Whether OCS received that command is not
    knowable from here: a demand appears in fastq-history only once it produces output, so
    a just-submitted job is indistinguishable from one that never ran.

Do not infer the outcome. Mark the entry STRANDED and leave it for a
    person, because the two mistakes are not equally bad: leaving a job unsubmitted costs a
    delay, while resubmitting one that is already running costs a duplicate alignment.
    Without this the row would sit in SUBMITTING forever, visible to nobody.
    """
