...
PLAN = reverse("submission_queue:queue-plan")
QUEUE = reverse("submission_queue:queue-list")
...
class TestPlanThenConfirm:
    def test_a_plan_can_be_read_then_confirmed_then_listed_then_cancelled(
        ...
        cancelled = api.post(reverse("submission_queue:queue-cancel", args=[entry_id]))
        assert cancelled.status_code == 200
        assert cancelled.json()["status"] == QueueEntry.Status.CANCELLED

    def test_a_plan_says_which_samples_are_carried_by_their_partner(self, api, active_config, make_sample):
        make_sample(
            "GEX-1",
    ...
    def test_forcing_a_stage_is_recorded_on_the_entry(self, api, active_config, make_sample):
        ...
        response = api.post(QUEUE, {"fastq_names": ["DONE-1"], "force": Stage.ALIGN.value}, format="json")
    ...
    def test_an_account_with_no_email_is_asked_for_one(self, active_config, make_sample, django_user_model):
...
class TestOtherPeoplesEntries:
    ...
    def test_you_cannot_read_it(self, api, theirs):
        assert api.get(reverse("submission_queue:queue-detail", args=[theirs.pk])).status_code == 404

    def test_you_cannot_cancel_it(self, api, theirs):
        response = api.post(reverse("submission_queue:queue-cancel", args=[theirs.pk]))
...
class TestEntriesAreReadOnly:
    def test_an_entry_cannot_be_edited_or_deleted_through_the_api(self, api, active_config, make_sample):
        make_sample("READY-1")
        api.post(QUEUE, {"fastq_names": ["READY-1"]}, format="json")
        entry = QueueEntry.objects.get()
        detail = reverse("submission_queue:queue-detail", args=[entry.pk])
...
class TestTheTwoDoorsAgree:
    ...
    def test_the_command_is_identical_through_the_api_and_the_page(
        ...
        page = logged_in.post(reverse("web_ui:submit-review"), {"fastq_names": ["READY-1"]})
    ...
    def test_a_skip_reason_is_identical_through_both(self, api, logged_in, active_config, make_sample):
        make_sample("WAITING-1", ingest=NOT_COMPLETED)

        from_api = api.post(PLAN, {"fastq_names": ["WAITING-1"]}, format="json").json()["skipped"][0]
        page = logged_in.post(reverse("web_ui:submit-review"), {"fastq_names": ["WAITING-1"]})
    ...
    def test_a_job_queued_through_the_api_shows_on_the_web_queue(
        ...
        entries = logged_in.get(reverse("web_ui:queue")).context["entries"]
    ...
    def test_neither_door_queues_the_same_stage_twice(self, api, logged_in, active_config, make_sample):
        make_sample("READY-1")
        logged_in.post(reverse("web_ui:submit-confirm"), {"fastq_names": ["READY-1"]})
