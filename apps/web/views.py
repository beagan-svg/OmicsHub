...
from apps.catalog.models import NOT_COMPLETED, BatchPrefix, Sample, Stage, StageStatus
from apps.catalog.services import pairing, sync
...
from apps.queueing.tasks import process_next_queue_entry
from apps.web import columns
from apps.web.forms import ConfigUploadForm, SubmissionForm, SyncForm
from apps.workflows import command_builder, modality, services
...
def dashboard(request):
    ...
    return render(request, "web/dashboard.html", _dashboard_context(request))
...
def sync_samples(request):
    ...
    return redirect(f"{reverse('web:dashboard')}?{query}")
...
def cart_add(request):
    ...
    return _cart_add_result(
        ...
        added=added,
        already=len(in_cart),
        # A page listing a sample the mirror no longer holds — someone else re-synced the
...
def checkout(request):
    ...
    return render(request, "web/checkout.html", _checkout_context(request))
...
def submit_review(request):
    ...
    if context is None:
        return redirect("web:checkout")
    return render(request, "web/checkout.html", {**context, "open_modal": "submit"})


@login_required
@require_POST
def command_preview(request):
    """Rebuild one fastq sample's command from the editor values.

    The command is assembled from the config on the server, so a menu in the editor cannot
show its own effect. This is the only honest way to display what a changed Reference produces
    is to ask for it. This did that by re-posting the whole form, which re-rendered the page
    and reopened the modal: a full navigation to change one field, and every open editor
    snapped shut on the way through.

    So the editor asks for the one command it needs instead. The form is posted as it
    stands, which is what carries the config, the email and every other override, and the
    answer is JSON the editor writes into the textarea in place.

    Plan a single sample. Write no queue entry and do not change the wider submission
    here. The real steps re-plan the whole selection from the same service, so a
    preview that went stale cannot leak into what gets queued.
    """
...
def submit_commands(request):
    ...
    if context is None:
        return redirect("web:checkout")
    return render(request, "web/checkout.html", {**context, "open_modal": "final"})
...
def submit_confirm(request):
    ...
    with transaction.atomic():
        ...
        # Only what actually reached the queue leaves the cart. A sample that was skipped —
        # ingest still running, say — stays staged so it can be submitted once it is ready,
...
def queue(request):
    entries = list(
        _owned(request).filter(status__in=QUEUED_STATUSES).prefetch_related("sample__stage_statuses")
    )
    next_entry = next((e for e in entries if e.status == QueueEntry.Status.PENDING), None)
    return render(request, "web/queue.html", {"entries": entries, "next_entry": next_entry})


@login_required
@require_POST
def process_now(request):
    """Ask the worker to submit the next queue entry before the next scheduled tick.

    The worker still honours both holds, so say so rather than reporting that something
    was asked for and leaving the queue apparently ignoring it.
    """
    process_next_queue_entry.delay()
    if cache.get(tasks.CAPACITY_HOLD_KEY):
        messages.info(request, "OCS is at its job limit. The queue starts again when capacity frees.")
    elif cache.get(tasks.SPACING_HOLD_KEY):
        messages.info(request, "Waiting out the spacing after the last submission.")
    else:
        messages.info(request, "Asked the worker to process the next job.")
    return redirect("web:queue")
...
def job_monitor(request):
    """What OCS is running and what it has finished, whoever submitted it.

    Read from the mirror — `StageStatus`, which the sweeps fill from DynamoDB — and not
    from this app's queue. A demand submitted by somebody running `ocs` by hand, or by the
    tool this replaced, is as real as one this app queued, and a monitor that only knows
    its own submissions reports an idle pipeline while OCS is busy. It also stops showing
    a job the moment its queue entry is deleted, which is the wrong way round: the queue
    entry is a record of asking, and this page is about what is actually happening.

    Whether OmicsHub asked for a demand is still worth knowing, so it is a column rather
    than a filter: `queued_here` marks the rows that came from this app's queue.
    """
    ...
    finished = list(
        stages.filter(status__in=FINISHED_OCS_STATUSES).order_by(
            F("last_update_time").desc(nulls_last=True)
        )[:MONITOR_ROWS]
    )
    ...
    return render(
        ...
        "web/job_monitor.html",
        {
            ...
            "counts": {
                "align": sum(1 for row in running if row.stage == Stage.ALIGN),
                "post_align": sum(1 for row in running if row.stage == Stage.POST_ALIGN),
                "total": len(running),
            },
...
def failed_jobs(request):
    entries = _owned(request).filter(status__in=FAILED_STATUSES).prefetch_related("sample__stage_statuses")
    # The stranded warning is the most consequential thing on the page, so it sits above the
    # table — which means the template has to know whether any row warrants it. Evaluated
    ...
    return render(
        ...
        {"entries": entries, "has_stranded": stranded},
    )
...
def retry_job(request, pk):
    """Create a queue entry for a failed job."""
    entry = get_object_or_404(_owned(request), pk=pk)
    if entry.status == QueueEntry.Status.STRANDED:
    ...
    elif QueueEntry.objects.filter(pk=entry.pk, status=QueueEntry.Status.FAILED).update(
...
def configs(request):
    ...
    return render(
        request, "web/configs.html", {"configs": WorkflowConfig.objects.select_related("uploaded_by")}
    )
...
def _filtered_samples(request):
    """Return rows selected by the current filters.

    The dashboard and the CSV export both go through here, which is the point: an export
    that built its own queryset would eventually disagree with the table it came from, and
    the user would have no way to tell which one was wrong.
    """
    ...
    if search:
        queryset = queryset.filter(fastq_name__icontains=search)
...
def _dashboard_context(request):
    ...
    return {
        ...
        # How often the two sweeps run, for the tooltip on the freshness clock — otherwise
        ...
        # which in practice means the beat scheduler or the worker is not running — the one
...
Order this to match the table's default sort. If the menu and the rows below it disagree,
    about what "first" means is its own small confusion.

    A sync that returned nothing still redirects here with its batch in the query string,
keep the selected value even when no sample carries it. Otherwise the menu would
    read "All batches" while the empty table below it is still filtered.
    """
...
Return None after messaging the user when there is nothing to plan.
    ...
    return {
        **_checkout_context(request),
        ...
        # dismiss without reading — which is what makes the prompt useless when it matters.
...
def _batch_groups(plan, stage: str, config: dict) -> list[dict]:
    ...
    for entry in plan.entries:
        if entry.stage != stage:
            continue
        batch = entry.sample.batch_name_from_vendor or "—"
        ...
        group["entries"].append(
            {
                ...
                # actually substitutes — no Reference menu above a post-QC command, which
...
def _safe_next(request, fallback="web:dashboard"):
