...
from apps.catalog.models import NOT_COMPLETED, Sample, Stage
from apps.catalog.services import sync
from apps.ocs import dynamodb
from apps.queueing.models import CartItem, QueueEntry
from apps.web import columns as column_defs
...
def other_user(db, django_user_model):
    """A second ordinary account. Not staff — staff can see everyone's queue by design."""
...
def submit(logged_in):
    ...
    def _submit(fastq_names, **extra):
        return logged_in.post(
            reverse("web:submit-confirm"), {"fastq_names": list(fastq_names), **extra}, follow=True
        )
...
def review(logged_in):
    ...
    def _review(fastq_names, **extra):
        return logged_in.post(
            reverse("web:submit-review"), {"fastq_names": list(fastq_names), **extra}
        )
...
def queued_entry(logged_in, active_config, make_sample):
    ...
    def _queued(fastq_name="READY-1", *, owner_client=None, **sample_kwargs):
        ...
        (owner_client or logged_in).post(reverse("web:submit-confirm"), {"fastq_names": [fastq_name]})
...
class TestSignedOut:
    ...
    PAGES = ["web:dashboard", "web:queue", "web:failed", "web:checkout", "web:configs"]
    ...
    def test_a_post_only_endpoint_also_preserves_where_you_were(self, client):
        response = client.post(reverse("web:cart-add"), {"fastq_names": ["READY-1"]})

        assert response.status_code == 302
        assert response["Location"] == f"{reverse('login')}?next={reverse('web:cart-add')}"
...
class TestDashboardFilters:
    ...
    def test_a_combination_that_matches_nothing_is_an_empty_table_not_an_error(self, logged_in):
        response = logged_in.get(
            reverse("web:dashboard"), {"organism_common_name": "human", "batch_prefix": "MTX"}
        )
    ...
    def test_the_not_completed_status_finds_stages_ocs_never_ran(self, logged_in):
        response = logged_in.get(reverse("web:dashboard"), {"ingest_status": NOT_COMPLETED})

        assert self.rows(response) == {"RAT-RFX"}

    def test_an_unknown_family_tab_shows_everything_rather_than_nothing(self, logged_in):
        response = logged_in.get(reverse("web:dashboard"), {"batch_prefix": "ZZZ"})
    ...
    def test_a_study_the_mirror_has_never_heard_of_returns_nothing(self, logged_in):
        response = logged_in.get(reverse("web:dashboard"), {"study": "NoSuchStudy"})
    ...
    def test_a_study_that_has_just_been_synced_can_be_filtered_on(
        ...
        `_upsert_samples` runs — it is the thing that invalidates the cached menu, and a
        stub in front of it would be testing the stub.
        """
        assert logged_in.get(reverse("web:dashboard")).context["studies"] == ["StudyA"]
        ...
        logged_in.post(reverse("web:sync"), {"batch_name_from_vendor": "MTX-999"}, follow=True)

        response = logged_in.get(reverse("web:dashboard"))
        assert response.context["studies"] == ["StudyA", "StudyB"]
        assert self.rows(logged_in.get(reverse("web:dashboard"), {"study": "StudyB"})) == {"FRESH-1"}

    def test_the_search_box_keeps_what_was_typed(self, logged_in):
        response = logged_in.get(reverse("web:dashboard"), {"fastq_name": "HUMAN"})
...
class TestPagingAndSorting:
    ...
    def test_page_beyond_the_end_shows_the_last_page(self, logged_in, many):
        response = logged_in.get(reverse("web:dashboard"), {"page": "999"})
    ...
    def test_a_page_number_that_is_not_a_number_shows_the_first_page(self, logged_in, many):
        response = logged_in.get(reverse("web:dashboard"), {"page": "abc"})

        assert response.status_code == 200
        assert response.context["page"].number == 1

    def test_an_unknown_sort_key_falls_back_to_the_default(self, logged_in, many):
        response = logged_in.get(reverse("web:dashboard"), {"sort": "'; DROP TABLE"})
    ...
    def test_every_sortable_column_sorts_both_ways(self, logged_in, make_sample, key, direction):
        ...
        response = logged_in.get(reverse("web:dashboard"), {"sort": key, "dir": direction})
    ...
    def test_sorting_by_batch_reads_the_number_not_the_text(self, logged_in, make_sample):
        ...
        response = logged_in.get(
            reverse("web:dashboard"), {"sort": "batch_name_from_vendor", "dir": "desc"}
        )
    ...
    def test_paging_keeps_the_filters(self, logged_in, make_sample):
        ...
        response = logged_in.get(
            reverse("web:dashboard"), {"organism_common_name": "human", "page": "2"}
        )
    ...
    def test_the_pager_link_carries_the_filters_and_the_sort(self, logged_in, make_sample):
        ...
        pager = [
            line
            for line in messages_in(response).splitlines()
            if "pager__step" in line and "href=" in line
        ]
    ...
    def test_re_sorting_drops_the_page_so_you_land_at_the_top(self, logged_in, make_sample):
        ...
        response = logged_in.get(reverse("web:dashboard"), {"page": "2"})
...
class TestColumnChoices:
    def test_a_chosen_set_of_columns_persists_for_that_user(self, logged_in, make_sample, user, client):
        ...
        logged_in.post(reverse("web:set-columns"), {"columns": ["fastq_name", "load_name"]})

        user.refresh_from_db()
        assert user.visible_columns == ["fastq_name", "load_name"]
        client.force_login(user)
        labels = [column.label for column in client.get(reverse("web:dashboard")).context["columns"]]
    ...
    def test_one_users_columns_do_not_follow_another_user(self, logged_in, other_client, make_sample):
        ...
        their_columns = other_client.get(reverse("web:dashboard")).context["columns"]
    ...
    def test_the_fastq_name_column_cannot_be_dropped(self, logged_in, make_sample, user):
        ...
        logged_in.post(reverse("web:set-columns"), {"columns": ["load_name", "modality"]})

        user.refresh_from_db()
        assert user.visible_columns[0] == "fastq_name"
        assert b"READY-1" in logged_in.get(reverse("web:dashboard")).content

    def test_clearing_every_box_still_leaves_a_readable_table(self, logged_in, make_sample, user):
        make_sample("READY-1")

        logged_in.post(reverse("web:set-columns"), {"columns": []})
...
class TestCartJourney:
    def test_the_badge_counts_what_is_staged(self, logged_in, make_sample):
        ...
        assert logged_in.get(reverse("web:dashboard")).context["cart_count"] == 2
    ...
    def test_add_the_same_sample_twice_then_remove_then_clear(self, logged_in, make_sample, user):
        ...
        again = logged_in.post(reverse("web:cart-add"), {"fastq_names": ["READY-1"]}, follow=True)
        assert b"1 already in the cart" in again.content
        assert CartItem.objects.filter(user=user).count() == 2

        removed = logged_in.post(reverse("web:cart-remove"), {"fastq_names": ["READY-1"]}, follow=True)
        assert b"Removed 1 sample from the cart" in removed.content
        assert logged_in.get(reverse("web:dashboard")).context["cart_count"] == 1

        cleared = logged_in.post(reverse("web:cart-clear"), follow=True)
    ...
    def test_a_fastq_name_the_mirror_no_longer_holds_is_reported(self, logged_in, make_sample, user):
        ...
        response = logged_in.post(
            reverse("web:cart-add"), {"fastq_names": ["READY-1", "GHOST-1"]}, follow=True
        )
    ...
    def test_a_cart_of_only_ghosts_says_so_rather_than_reporting_success(self, logged_in, user):
        response = logged_in.post(reverse("web:cart-add"), {"fastq_names": ["GHOST-1"]}, follow=True)
    ...
    def test_one_users_cart_is_their_own(self, logged_in, other_client, make_sample, other_user):
        ...
        assert other_client.get(reverse("web:checkout")).context["cart_items"] == []
...
class TestSubmissionJourney:
    def test_the_three_steps_lead_to_a_queued_job(self, logged_in, active_config, make_sample, user):
        ...
        checkout = logged_in.get(reverse("web:checkout"))
        assert [item.sample.fastq_name for item in checkout.context["cart_items"]] == ["READY-1"]

        step_one = logged_in.post(reverse("web:submit-review"), {"fastq_names": ["READY-1"]})
        ...
        step_two = logged_in.post(reverse("web:submit-commands"), {"fastq_names": ["READY-1"]})
        assert step_two.context["open_modal"] == "final"
        assert user.email.encode() in step_two.content

        step_three = logged_in.post(
            reverse("web:submit-confirm"), {"fastq_names": ["READY-1"]}, follow=True
        )
    ...
    def test_a_hand_edited_command_is_what_gets_queued(
    ...
    def test_a_skipped_sample_stays_in_the_cart_for_next_time(
        ...
        make_sample("READY-1")
        make_sample("WAITING-1", ingest=NOT_COMPLETED)
        logged_in.post(reverse("web:cart-add"), {"fastq_names": ["READY-1", "WAITING-1"]})

        logged_in.post(reverse("web:submit-confirm"), {"fastq_names": ["READY-1", "WAITING-1"]})
    ...
    def test_without_an_active_config_the_flow_says_what_is_missing(self, logged_in, make_sample):
        ...
        response = logged_in.post(
            reverse("web:submit-review"), {"fastq_names": ["READY-1"]}, follow=True
        )
    ...
    def test_previewing_a_sample_that_cannot_run_returns_the_reason(
        ...
        response = logged_in.post(
            reverse("web:command-preview"), {"fastq_name": "WAITING-1"}
        )
    ...
    def test_previewing_an_unknown_sample_is_a_404_not_a_500(self, logged_in, active_config):
        response = logged_in.post(reverse("web:command-preview"), {"fastq_name": "GHOST-1"})
...
class TestQueueAndFailures:
    def test_cancelling_a_pending_entry(self, logged_in, queued_entry):
        ...
        response = logged_in.post(reverse("web:cancel", args=[entry.pk]), follow=True)
    ...
    def test_retrying_a_failed_entry_puts_it_back_on_the_queue(self, logged_in, queued_entry):
        ...
        listing = logged_in.get(reverse("web:failed"))
    ...
    def test_the_failures_page_warns_above_the_table_when_anything_is_stranded(
        self, logged_in, queued_entry
    ):
        entry = queued_entry("READY-1")
        QueueEntry.objects.filter(pk=entry.pk).update(status=QueueEntry.Status.STRANDED)

        assert logged_in.get(reverse("web:failed")).context["has_stranded"] is True
    ...
    def test_deleting_a_failed_entry(self, logged_in, queued_entry):
        ...
        response = logged_in.post(reverse("web:delete-job", args=[entry.pk]), follow=True)
...
class TestOtherPeoplesJobs:
    ...
    def theirs(self, other_client, active_config, make_sample, other_user):
        ...
        other_client.post(reverse("web:submit-confirm"), {"fastq_names": ["THEIRS-1"]})
    ...
    def test_it_is_not_on_your_queue_page(self, logged_in, theirs):
        assert logged_in.get(reverse("web:queue")).context["entries"] == []
    ...
    def test_you_cannot_cancel_it(self, logged_in, theirs):
        response = logged_in.post(reverse("web:cancel", args=[theirs.pk]))
    ...
    def test_you_cannot_retry_it(self, logged_in, theirs):
        ...
        response = logged_in.post(reverse("web:retry", args=[theirs.pk]))
    ...
    def test_you_cannot_delete_it(self, logged_in, theirs):
        ...
        response = logged_in.post(reverse("web:delete-job", args=[theirs.pk]))
...
class TestSettingsPage:
    def test_an_ordinary_user_is_told_no_rather_than_bounced_to_sign_in(self, logged_in):
        response = logged_in.get(reverse("web:configs"))
    ...
    def test_an_ordinary_user_cannot_activate_a_config_either(self, logged_in, active_config, config, user):
        ...
        response = logged_in.post(reverse("web:activate-config", args=[spare.pk]))

        spare.refresh_from_db()
        assert response.status_code == 403
        assert spare.is_active is False

    def test_the_old_configs_path_still_lands_on_settings(self, staff_client):
        response = staff_client.get("/configs/")

        assert response.status_code == 302
        assert response["Location"] == reverse("web:configs")

    def test_staff_can_upload_a_config_and_it_arrives_inactive(self, staff_client, config):
        response = staff_client.post(
            reverse("web:configs"), {"file": config_upload(config, "good.jsonc")}, follow=True
        )
    ...
    def test_a_config_that_is_not_json_is_rejected_and_named(self, staff_client):
        ...
        response = staff_client.post(reverse("web:configs"), {"file": broken}, follow=True)
    ...
    def test_a_config_missing_sections_names_the_sections(self, staff_client):
        ...
        response = staff_client.post(reverse("web:configs"), {"file": thin}, follow=True)
    ...
    def test_a_command_written_as_a_string_is_rejected_with_the_fix(self, staff_client, config):
        ...
        response = staff_client.post(
            reverse("web:configs"), {"file": config_upload(config, "stringly.jsonc")}, follow=True
        )
    ...
    def test_a_file_that_is_not_utf8_text_is_rejected(self, staff_client):
        ...
        response = staff_client.post(reverse("web:configs"), {"file": binary}, follow=True)
    ...
    def test_an_oversized_file_is_refused_before_it_is_parsed(self, staff_client):
        ...
        response = staff_client.post(reverse("web:configs"), {"file": huge}, follow=True)

        assert b"Config files are under 2 MB" in response.content
        assert not WorkflowConfig.objects.exists()

    def test_uploading_nothing_is_refused(self, staff_client):
        response = staff_client.post(reverse("web:configs"), {}, follow=True)
    ...
    def test_activating_a_config_stands_the_previous_one_down(
        ...
        response = staff_client.post(
            reverse("web:activate-config", args=[replacement.pk]), follow=True
        )
    ...
    def test_a_newly_activated_config_is_what_the_next_submission_uses(
        ...
        staff_client.post(reverse("web:activate-config", args=[replacement.pk]))

        assert review(["READY-1"]).context["plan"].entries[0].command.startswith(
            "ocs fastqs align tenx-arc-next"
        )
...
class TestCsvExport:
    def test_it_exports_what_the_filters_are_showing(self, logged_in, make_sample):
        ...
        response = logged_in.post(f"{reverse('web:export')}?organism_common_name=human")
    ...
    def test_a_ticked_selection_narrows_the_export_further(self, logged_in, make_sample):
        ...
        response = logged_in.post(
            f"{reverse('web:export')}?organism_common_name=human", {"fastq_names": ["HUMAN-1"]}
        )
    ...
    def test_the_header_row_is_the_users_chosen_columns(self, logged_in, make_sample):
        ...
        header, first = csv_body(logged_in.post(reverse("web:export"))).splitlines()[:2]

        assert header == "Fastq Name,Load Name"
        assert first == "READY-1,LOAD_XYZ"

    @pytest.mark.parametrize("prefix", ["=", "+", "-", "@"])
    def test_a_value_a_spreadsheet_would_run_as_a_formula_is_defused(
        self, logged_in, make_sample, prefix
    ):
        make_sample("READY-1", batch_name_from_vendor=f"{prefix}HYPERLINK(\"http://evil\")")

        body = csv_body(logged_in.post(reverse("web:export")))

        assert f"'{prefix}HYPERLINK" in body

    def test_an_export_of_nothing_is_still_a_valid_file_with_headers(self, logged_in):
        body = csv_body(logged_in.post(f"{reverse('web:export')}?organism_common_name=nobody"))
...
class TestOddData:
    ...
    def test_the_batch_menu_and_the_table_agree_on_where_a_digitless_batch_goes(
        self, logged_in, make_sample
    ):
        make_sample("NODIGITS-1", batch_name_from_vendor="MTX-PILOT")
        make_sample("NUMBERED-1", batch_name_from_vendor="MTX-42")
    ...
    @pytest.mark.parametrize(
        "term", ["マウス", "café", "%", "_", "100%_", "'; DROP TABLE catalog_sample; --"]
    )
    def test_an_awkward_search_term_returns_nothing_rather_than_erroring(self, logged_in, make_sample, term):
        make_sample("READY-1")

        response = logged_in.get(reverse("web:dashboard"), {"fastq_name": term})
    ...
    def test_a_search_term_longer_than_the_column_is_survivable(self, logged_in, make_sample):
        ...
        response = logged_in.get(reverse("web:dashboard"), {"fastq_name": "x" * 5000})
    ...
    def test_a_unicode_batch_name_survives_the_round_trip(self, logged_in, make_sample):
        ...
        response = logged_in.get(
            reverse("web:dashboard"), {"batch_name_from_vendor": "MTX-Ω-22068"}
        )

        assert [s.fastq_name for s in response.context["page"].object_list] == ["UNI-1"]
        assert "MTX-Ω-22068" in csv_body(logged_in.post(reverse("web:export")))
    ...
    def test_refreshing_status_for_rows_that_are_gone_says_so(self, logged_in):
        response = logged_in.post(
            reverse("web:refresh-status"), {"fastq_names": ["GHOST-1"]}, follow=True
        )
