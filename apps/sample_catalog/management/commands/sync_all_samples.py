...
class Command(BaseCommand):
    help = (
        "Sync OCS samples whose batch-name-from-vendor prefix has "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Syncing batches for: {', '.join(sorted(prefixes))}")
        ...
        self.stdout.write(self.style.SUCCESS(f"Synced {result['synced']} samples."))
        if result["pruned"]:
