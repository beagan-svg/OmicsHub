"""Build an `ocs` command from a fastq sample and the workflow manifest.

The matching rules, ported unchanged from the OCS Submission Capsule
(`ocs_submission/ocs_command_builder.py`), which reads this same config format:

* Command configs are tried in file order and the first match wins, so more specific
  entries are placed above general ones in the config.
* A library prep that appears in no command config for the modality and stage means the
  stage simply does not run for that sample. Report this as expected, not an error.
* A library prep that *is* listed but whose organism is not covered is a config mistake
  and raises, because silently skipping it would hide the missing entry.
"""
...
def _normalise_organism(name: str) -> str:
    """Normalize case and separators so organism names match.

    OCS is not consistent about its separator. The mirror holds `macaque-nemestrina` and
    `harbor_porpoise`, and a manifest written against one spelling silently failed against
    the other. The config could carry both, and for `harbor-porpoise | harbor_porpoise` it
    already does, but that only works for the variants somebody remembered.
    """
    return name.replace("_", "-").casefold()


def organism_entry(mapping: dict, organism_common_name: str, *, default=None):
    """Return an organism entry from a manifest section, ignoring `-` versus `_` and case.

    An exact key always wins, so a manifest that spells two organisms differently keeps
    both entries. The fold is only consulted when there is no exact
    match, and a fold that lands on two entries carrying *different* values is an error
    rather than a coin toss. Guessing in this case means aligning against the
    wrong genome.
    """
