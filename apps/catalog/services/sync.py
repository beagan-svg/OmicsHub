...
Key samples by `fastq_name` and statuses by `(sample, stage)`. Write both with an upsert so
re-running a sync updates rows instead of duplicating them.
"""
...
from apps.catalog.models import Sample, Stage, StageStatus, prefixes_for_workflows
...
# records demand types beyond these — "transfer", "workflow", "null" — which the dashboard
...
STUDY_OPTIONS_KEY = "web:study-options"
...
# updating. Not an OCS status — OCS has no such label, which is the whole problem: those
...
def _upsert_samples(entries: list[dict]) -> list[Sample]:
    """Write one page of fastq-metadata entries to the mirror and return the rows.

    The one place a metadata entry becomes a Sample. On Postgres an upserting bulk_create
    returns the instances with their primary keys, which is what lets callers go straight
    on to `sync_stage_statuses` with them.
    """
...
def _in_scope(entries: list[dict], batch_prefixes: set[str]) -> list[dict]:
    ...
    Items missing either key attribute are dropped rather than indexed: DynamoDB items are
    schemaless, and one entry without `batch_name_from_vendor` used to raise KeyError and
    abandon the rest of the page and every later page during the sweep.
    """
    wanted = []
...
def active_batch_prefixes() -> set[str] | None:
    """Return vendor prefixes in the active manifest, or None without a manifest.

    Use more than workflow names because ATX defines no workflow of its own but has to be
    mirrored whenever MTX is configured, because an MTX submission cannot align without its
    ATAC half. Scoping by workflow name alone means the sweep never fetches ATX and
    worse, prunes the ATX samples already mirrored.

    None rather than an empty set, because the two mean opposite things: an empty scope
    reaches `_prune_out_of_scope` as "delete the entire mirror".
    """
...
def sync_all_samples(batch_prefixes: set[str], progress=None) -> dict[str, int]:
    """Load fastq-metadata entries whose batch has a workflow in the active manifest.

    The table holds hundreds of thousands of entries, the vast majority from pipelines
    this app cannot submit. They have no OCS ingest, so alignment can never start. Scoping
    the mirror to configured workflows keeps every row on the dashboard actionable.

    Do not fetch stage status here. fastq-history is keyed by fastq name, so
    it would mean one query per sample. `sync_all_stage_statuses` sweeps it separately.

    Safe to re-run. Conflicts on `fastq_name` update the row rather than duplicating it.
    """
    total, skipped = 0, 0
...
def _prune_out_of_scope(batch_prefixes: set[str]) -> int:
    """Delete mirrored samples whose batch no longer has a workflow in the manifest.

    Without this, narrowing the config would leave rows nobody can act on sitting in the
    dashboard forever. Samples with queue history are left alone: their queue entry is the
    only record that the job was ever submitted.
    """
    if not batch_prefixes:
        # exclude(Q()) contributes no WHERE clause, so an empty scope would match — and
        # delete — every sample. A scope this app cannot name is a config fault, not a
...
def sync_all_stage_statuses(batch_prefixes: set[str]) -> dict[str, int]:
    ...
    def offer(fastq_name: str, stage: str, demand: dict) -> None:
        """Keep the demand that best describes a fastq sample and stage.

        Case is folded here so both feeds get it: the history half arrives lowercased, the
    registry's `demand_type` is the value OCS wrote, and a stage that misses
        TRACKED_STAGES on case alone is dropped from the dashboard without a word.
        """
        stage = stage.lower()
        # OCS runs demand types this app does not model — "transfer", for one. Storing
    ...
    # History covers the stages whose demands do not name their fastqs — post-alignment
    # and ingest — and confirms completed alignments.
    #
    ...
    # minutes, whether or not OCS changed anything — which was ~13M row-writes a day to
...
def sync_stage_statuses(samples: list[Sample]) -> None:
    ...
    for (sample_id, stage), candidate_ids in candidates.items():
        # A history row can outlive its demand — the registry is pruned, history is not.
