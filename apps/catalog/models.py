...
class BatchPrefix(models.TextChoices):
    ...
    Keep this separate from `Modality` because ATX is not its own workflow. It is the ATAC
    half of a multiome experiment and runs under MTX, but it still has to be selectable
    on its own, which a single field collapsing ATX into MTX could not do.
    """
...
class Stage(models.TextChoices):
    """Define pipeline stages using their OCS demand types.

    Storing the OCS value ("post-align", not "postqc") means history rows read from
    fastq-history map onto these without a translation table.

    EXPORT is the BKP Codex step that follows post-alignment. It is *observed*, never
    submitted: the export is triggered downstream, by a PTS notification into an Airflow
    DAG in another account, and this app does not queue one. It appears in this enum because
    OCS records it as a demand type like any other (`gcs_core.models.DemandType.EXPORT`),
    so its status, demand id, duration and file store id arrive through the same
    fastq-history sweep as the other three and need no second integration.

    Two consequences worth naming, because both are load-bearing:

    * `sync.TRACKED_STAGES` is derived from this enum, so adding the member is what makes
      the sweep start storing export rows instead of discarding them.
    * The submission side indexes `COMPLETE_STATUS_KEY` and `COMMAND_CONFIG_FIELD` by an
      explicit stage and never by iterating this enum, so nothing tries to plan or queue an
      export. Keep it that way: an export has no command config, and adding one would mean
      submitting a job OCS does not accept from here.
    """
...
class Sample(models.Model):
    ...
    Synced from DynamoDB rather than owned here: OCS is the source of truth, this copy
    exists so samples can be listed, filtered, and queued against without a DynamoDB
    ...
    #
    # Everything not listed — the bare 10X* batches included — falls through to RTX. There
    ...
    def stage_record(self, stage: str):
        """Return the mirrored StageStatus for one stage, or None without an OCS record.

        Iterates the prefetched relation rather than filtering it, because filtering a
        prefetched queryset issues a fresh query per call. On a fifty-row table
        with three stage columns is a hundred and fifty queries instead of none.
        """
    ...
    def stage_status(self, stage: str) -> str:
        """Return the OCS status label for a stage, or NOT COMPLETED when none is known.

        Reads from `stage_statuses`, so callers that need this for many samples should
        prefetch that relation.
        """
...
class StageStatus(models.Model):
    """Store the latest OCS demand for one fastq sample and one stage.

    Mirrors the join of fastq-history (which demand ran) and demand-registry (how it
    ended). Replaces the old app's separate ingest/alignment/postqc tables.
    """
