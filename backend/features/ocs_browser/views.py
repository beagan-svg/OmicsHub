...
from django.db.models import Prefetch, Q, QuerySet, F, Count, Exists, OuterRef
...
from backend.core.models import Main, Metadata, LoadAssociation, Ingest, Alignment, PostQC
from .sample_filters import MainFilter, DISTINCT_ON_SUPPORTED
...
class ResponseOptimizationMixin:
    ...
    def optimize_sample_data(self, sample) -> Dict[str, Any]:
        ...
        return {
            ...
            'batch_name': sample.fastq_name.batch_name if sample.fastq_name else '',
...
class ProductionMainListView(FilterView, DatabaseOptimizationMixin, ResponseOptimizationMixin):
    """
...
class DatabaseIndexManager:
    """Utility class for managing database indexes for optimal performance"""
    ...
    def get_recommended_indexes():
        ...
        return [
            # Main table indexes
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_study_set ON main(study_set) WHERE study_set IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_library_prep_method ON main(library_prep_method) WHERE library_prep_method IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_alignment_status ON main(alignment_status) WHERE alignment_status IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_postqc_status ON main(postqc_status) WHERE postqc_status IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_ingest_status ON main(ingest_status) WHERE ingest_status IS NOT NULL;",
            
            # Metadata table indexes
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_metadata_organism_common_name ON metadata(organism_common_name) WHERE organism_common_name IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_metadata_batch_name_from_vendor ON metadata(batch_name_from_vendor) WHERE batch_name_from_vendor IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_metadata_library_prep_method_name ON metadata(library_prep_method_name) WHERE library_prep_method_name IS NOT NULL;",
            
            # Composite indexes for common filter combinations
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_main_status_composite ON main(alignment_status, postqc_status, ingest_status);",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_metadata_search_composite ON metadata(organism_common_name, batch_name_from_vendor, library_prep_method_name);",
            
            # LoadAssociation indexes
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_load_association_load_name ON load_association(load_name) WHERE load_name IS NOT NULL;",
            
            # Status table indexes for FID lookups
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ingest_fid ON ingest(fid) WHERE fid IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_alignment_fid ON alignment(fid) WHERE fid IS NOT NULL;",
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_postqc_fid ON postqc(fid) WHERE fid IS NOT NULL;",
        ]
